...
export type PreparedAreaSearchOption<T extends AreaSearchSource> = {
  area: T;
...
export function prepareAreaSearchOptions<T extends AreaSearchSource>(
  areas: T[],
): PreparedAreaSearchOption<T>[] {
  return areas.map((area, originalIndex) => ({
    area,
    originalIndex,
    fields: [
      { value: normalizeAreaSearchValue(area.name), fieldRank: 0 },
      ...(area.nameEn
        ? [{ value: normalizeAreaSearchValue(area.nameEn), fieldRank: 1 }]
        : []),
...
export function rankPreparedAreaSearchOptions<T extends AreaSearchSource>(
  preparedAreas: PreparedAreaSearchOption<T>[],
  ...
  return preparedAreas
    .map((preparedArea) => ({
      ...preparedArea,
      score: getBestMatchScore(preparedArea.fields, normalizedQuery),
    }))
    .filter(
      (
        result,
